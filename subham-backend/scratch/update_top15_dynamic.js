...
function generateSeoForProduct(p) {
  ...
  if (lowerTitle.includes('parikshadham') && lowerTitle.includes('prabandhan')) {
  ...
  } else if (lowerTitle.includes('punekar') && lowerTitle.includes('mpsi')) {
  ...
  } else if (lowerTitle.includes('exampedia') && lowerTitle.includes('prabandhan')) {
  ...
  } else if (lowerTitle.includes('punekar') && lowerTitle.includes('group 2') && lowerTitle.includes('notes')) {
  ...
  } else if (lowerTitle.includes('geography') && lowerTitle.includes('shubham gupta') && lowerTitle.includes('english')) {
  ...
  } else if (lowerTitle.includes('general science') && lowerTitle.includes('dev yadav')) {
  ...
  } else if (lowerTitle.includes('आधुनिक भारतीय इतिहास') || (lowerTitle.includes('modern indian history') && lowerTitle.includes('hindi'))) {
  ...
  } else if (lowerTitle.includes('punekar') && lowerTitle.includes('solved papers')) {
  ...
  } else if (lowerTitle.includes('pariksha dham') && lowerTitle.includes('general studies') || lowerTitle.includes('parikshadham') && lowerTitle.includes('mpgk')) {
  ...
  } else if (lowerTitle.includes('exampedia') && lowerTitle.includes('pyqs') && lowerTitle.includes('paper-1')) {
  ...
  } else if (lowerTitle.includes('प्राचीन भारतीय इतिहास')) {
  ...
  } else if (lowerTitle.includes('geography') && lowerTitle.includes('shubham gupta') && lowerTitle.includes('hindi')) {
    ...
    metaTitle = 'Indian Geography (भारत का भूगोल) Spiral Notes by Shubham Gupta Sir | Hindi — Shubham Xerox';
  ...
  } else if (lowerTitle.includes('computer') && lowerTitle.includes('parikshadham')) {
  ...
  } else if (lowerTitle.includes('modern indian history') && lowerTitle.includes('english')) {
  ...
  } else if (lowerTitle.includes('polity') && lowerTitle.includes('laxmikanth')) {
  ...
  return { newSlug, author, publisher, tags, seo: { metaTitle, metaKeywords, metaDescription } };
}
...
async function run() {
  ...
  console.log(`Processing top ${top15.length} products dynamically...\n`);

  for (let i = 0; i < top15.length; i++) {
    const p = top15[i];
    const seoData = generateSeoForProduct(p);

    const oldSlugs = p.oldSlugs || [];
    if (p.slug !== seoData.newSlug && !oldSlugs.includes(p.slug)) {
      oldSlugs.push(p.slug);
    }

    const updatePayload = {
      slug: seoData.newSlug,
      oldSlugs: oldSlugs,
    ...
    console.log(`[${i + 1}/15] Product ID: ${updated._id}`);
    console.log(`  Title: ${updated.title}`);
    console.log(`  Old Slug: ${oldSlugs.join(', ')}`);
    console.log(`  New Canonical Slug: ${updated.slug}`);
    console.log(`  New Canonical URL: https://shubhamxerox.in/product/${updated.slug}`);
    console.log(`  Meta Title: ${updated.seo?.metaTitle}`);
    console.log(`  Tags (${updated.tags.length}): ${updated.tags.slice(0, 5).join(', ')}...`);
    console.log('--------------------------------------------------');
  }

  console.log('\n🎉 ALL TOP 15 PRODUCTS DYNAMICALLY UPDATED & VERIFIED IN DATABASE!');
