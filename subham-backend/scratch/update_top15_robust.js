...
async function run() {
  ...
  for (let i = 0; i < top15.length; i++) {
    ...
    } else if (lower.includes('geography') && (lower.includes('hindi') || lower.includes('b\n') || lower.includes('hindi medium'))) {
    ...
    if (p.slug !== newSlug && !oldSlugs.includes(p.slug)) {
    ...
    report.push({
      ...
      oldUrl: `https://shubhamxerox.in/product/${p.slug}`,
      newUrl: `https://shubhamxerox.in/product/${updated.slug}`,
      metaTitle: updated.seo?.metaTitle,
      tagsCount: updated.tags?.length || 0
    });
  }

  console.log('=== UPDATE COMPLETE REPORT ===');
  report.forEach((r) => {
    console.log(`[#${r.index}] ${r.title.slice(0, 45)}...`);
    console.log(`  Old URL: ${r.oldUrl}`);
    console.log(`  New Canonical URL: ${r.newUrl}`);
    console.log(`  Meta Title: ${r.metaTitle}`);
    console.log('---');
  });
